...
static void page(AsyncWebServerRequest *request) {
    ...
    if (request->hasArg("pocsag_telegram_type")) {
        pocsag_telegram_type =
            (func_t)request->arg("pocsag_telegram_type").toInt();
    }
    ...
    if (pager_number > 0 || force) {
        ...
        AsyncWebServerResponse *response =
            request->beginResponse(200, "text/plain", str);

        response->addHeader("Access-Control-Allow-Origin", "*");
        request->send(response);
        if (multi_pager_types) {
            for (int i = 0; i < 16; i++) {
                if (multi_pager_types & (1 << i)) {
                    call_pager(i, tx_power, tx_frequency, tx_deviation,
                               pocsag_baud, restaurant_id, system_id,
                               pager_number, alert_type, reprog,
                               pocsag_telegram_type, message.c_str(), cancel);
                }
            }
        } else {
            call_pager(mode, tx_power, tx_frequency, tx_deviation, pocsag_baud,
                       restaurant_id, system_id, pager_number, alert_type,
                       reprog, pocsag_telegram_type, message.c_str(), cancel);
        }
    } else {
